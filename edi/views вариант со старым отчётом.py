...
def daily_report(request):
    """GET /daily-report/ — ежедневный отчёт, сгруппированный по месту доставки."""
    ...
    utc_tz       = ZoneInfo('UTC')
    today_almaty = timezone.now().astimezone(almaty_tz).date()

    report_date_str = request.GET.get('date', '')
    if not report_date_str:
        last_doc = EdiDocument.objects.filter(doc_type='ORDER').order_by('-received_at').first()
        report_date = last_doc.received_at.astimezone(almaty_tz).date() if last_doc else today_almaty
    else:
        try:
            report_date = date.fromisoformat(report_date_str)
        except ValueError:
            report_date = today_almaty
    ...
    for doc in docs:
        ...
        for pos in raw.get('positions', []):
            ...
            g['name']     = pos.get('name', '')
            g['qty']     += float(pos.get('qty', 0))
            price_vat     = float(pos.get('sum_vat') or 0)
            qty_val       = float(pos.get('qty', 0))
            g['amount']  += (price_vat * qty_val) if price_vat else float(pos.get('amount', 0))

    # Преобразуем в список групп для шаблона
    # [{ 'place_name': ..., 'place_addr': ..., 'rows': [...], 'subtotal_qty': ..., 'subtotal_amount': ... }]
    place_groups = []
    total_qty    = 0.0
    total_amount = 0.0
    ...
    # Сортируем группы по названию точки
    place_groups.sort(key=lambda g: g['place_name'])

    if request.GET.get('export') == 'xlsx':
        return _daily_report_xlsx(report_date, place_groups, total_qty, total_amount)
...
def _daily_report_xlsx(report_date, place_groups, total_qty, total_amount):
    """Excel-экспорт ежедневного отчёта, сгруппированного по месту доставки."""
    import openpyxl, io
    ...
    # Цвета
    C_HEADER  = '1E3A5F'   # шапка таблицы
    C_PLACE   = '1E3A5F'   # строка-группа (место доставки) — тёмно-синяя
    C_PLACE_FG= 'FFFFFF'
    C_SUB     = 'E8F0FE'   # промежуточный итог
    C_ALT     = 'F0F4FA'   # чередование строк
    C_TOTAL   = 'DBEAFE'   # итого
    C_BLUE    = '2563EB'
    ...
    thin   = border()
    COLS   = 4  # Артикул | Наименование | Кол-во | Сумма
    WIDTHS = [18, 52, 18, 22]
    HEADERS= ['Артикул', 'Наименование продукта', 'Кол-во', 'Сумма с НДС, ₸']

    # ── Заголовок документа ──
    ws.merge_cells('A1:D1')
    ws['A1'] = f'Ежедневный отчёт — {report_date.strftime("%d.%m.%Y")}'
    ws['A1'].font      = Font(name='Arial', bold=True, size=14, color=C_HEADER)
    ...
    for col, (title, width) in enumerate(zip(HEADERS, WIDTHS), 1):
        ...
        c.fill = HFILL; c.font = HFONT; c.alignment = HALIGN; c.border = thin
    ...
    for group in place_groups:
        # ── Строка места доставки ──
        PFILL = PatternFill('solid', fgColor='2D4A7A')
        PFONT = Font(name='Arial', bold=True, color=C_PLACE_FG, size=10)
        ...
        for i, pos in enumerate(group['rows']):
            ...
            for col, val, align, fmt in [
                (1, pos['article'], A_C,  None),
                (2, pos['name'],    A_L,  None),
                (3, pos['qty'],     A_C,  '#,##0.###'),
                (4, pos['amount'],  A_R,  '#,##0.00'),
            ]:
        ...
        # ── Промежуточный итог группы ──
        SFILL = PatternFill('solid', fgColor=C_SUB)
        SFONT = Font(name='Arial', bold=True, size=9, color=C_BLUE)
        ws.merge_cells(f'A{row_n}:B{row_n}')
        ws[f'A{row_n}'] = f'Итого по точке: {group["place_name"]}'
        ...
        row_n += 2  # пустая строка между группами

    # ── Итого по всему отчёту ──
    TFILL = PatternFill('solid', fgColor=C_TOTAL)
    TFONT = Font(name='Arial', bold=True, size=11, color=C_BLUE)
    ws.merge_cells(f'A{row_n}:B{row_n}')
    ...
    buf = io.BytesIO()
    wb.save(buf); buf.seek(0)
    fname = f'daily_report_{report_date.strftime("%Y%m%d")}.xlsx'
