    ...
    def render(self, doc_data: dict) -> str:
        ...
        if self.position_tpl:
            ...
            for i, p in enumerate(doc_data.get('positions', []), start=1):
                ...
                line = line.replace('{{item_code}}',       str(p.get('article_buyer') or p.get('article_sup') or p.get('ean', '')))
                line = line.replace('{{item_name}}',       str(p.get('name', '')))
                line = line.replace('{{quantity}}',        str(p.get('qty', 0)))
                line = line.replace('{{unit_price}}',      str(p.get('price', 0)))
                line = line.replace('{{vat}}',             str(p.get('vat_rate', 0)))
                line = line.replace('{{amount}}',          str(p.get('amount', 0)))
                line = line.replace('{{amount_with_vat}}', str(p.get('sum_vat', 0)))
                lines.append(line)
            positions_xml = '\n'.join(lines)
